//! If the environment variable `SLD_REFERENCE_LINKER` is set, then once we've finished linking,
...
fn run_diff(paths: &BinPaths) -> Result {
    ...
    if !linker_diff_path.exists() {
        bail!("linker-diff binary needs to be in the same directory as sld")
    }
