...
impl LoadedPlugin {
    fn new(plugin_path: &Path, args: &ElfArgs) -> Result<LoadedPlugin> {
        ...
        transfer_vector.push(LdPluginTv {
            ...
            value: sld_plugin_message_callback as *const () as usize,
        });
...
unsafe extern "C" {
    ...
    /// then calls `sld_handle_plugin_message` with the resulting string. Defined in
    /// `plugin_message_shim.c`.
    fn sld_plugin_message_callback(level: libc::c_int, fmt: *const libc::c_char, ...);
}

/// Called by the C shim `sld_plugin_message_callback` with the already-formatted message string.
/// The `no_mangle` is required so the C shim can link against it by name.
#[unsafe(no_mangle)]
extern "C" fn sld_handle_plugin_message(level: libc::c_int, message: *const libc::c_char) {
