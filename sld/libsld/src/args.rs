...
/// inconsistency.
pub(crate) const WRITE_VERIFY_ALLOCATIONS_ENV: &str = "SLD_VERIFY_ALLOCATIONS";
...
impl Default for CommonArgs {
    fn default() -> Self {
        Self {
            ...
            print_allocations: std::env::var("SLD_PRINT_ALLOCATIONS")
                .ok()
...
mod tests {
    ...
    #[test]
    fn test_flavor() {
        let args = Args::new(|| ["ld.sld"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));

        let args = Args::new(|| ["ld64.sld"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));

        let mut args = Args::new(|| ["sld", "-flavor", "gnu"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));
        args.parse(|| ["sld", "-flavor", "gnu"].into_iter())
            .unwrap();
        assert!(args.common().inputs.is_empty());

        let args = Args::new(|| ["sld", "-flavor", "darwin"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));

        // -flavor has priority
        let args = Args::new(|| ["ld.sld", "-flavor", "darwin"].into_iter()).unwrap();
        assert!(matches!(args, Args::MachO(_)));

        let args = Args::new(|| ["ld64.sld", "-flavor", "gnu"].into_iter()).unwrap();
        assert!(matches!(args, Args::Elf(_)));

        assert!(Args::new(|| ["ld.sld", "-flavor", "invalid"].into_iter()).is_err());
        assert!(Args::new(|| ["ld.sld", "-flavor"].into_iter()).is_err());
    }
    ...
    fn no_incremental_overrides_incremental_env() {
        with_env_var(INCREMENTAL_ENV, Some("1"), || {
            let mut args = Args::new(|| ["sld", "-flavor", "gnu"].into_iter()).unwrap();
            args.parse(|| ["sld", "-flavor", "gnu", "--no-incremental"].into_iter())
                .unwrap();
