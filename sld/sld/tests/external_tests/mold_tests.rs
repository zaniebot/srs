...
// Therefore, we have to implement a similar filter for the sld tests as well.
fn should_skip_mold_test_by_arch(path: &Path) -> bool {
    ...
    let cross_archs = get_sld_test_cross().unwrap().unwrap_or_default();
