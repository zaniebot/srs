...
```sh
# For aarch64
SLD_TEST_CROSS=aarch64 cargo test

# For riscv64
SLD_TEST_CROSS=riscv64 cargo test

# For loongarch64
SLD_TEST_CROSS=loongarch64 cargo test

# All
SLD_TEST_CROSS=all cargo test
```
...
- `qemu_arch`: List of additional architectures (different from the host) to run tests for. This
  setting is overridden by the `$SLD_TEST_CROSS` environment variable. The default value is
  `[]`.
...
```sh
SLD_TEST_CONFIG=path_to_config cargo test
```
...
```sh
clang-format -i sld/tests/sources/*.{c,cc,h}
```
...
When working on tests, you can temporarily disable the formatting check by setting the `SLD_TEST_IGNORE_FORMAT` environment variable.
...
When debugging a failing test, it can be useful to see how other linkers (such as GNU ld or lld) behave on the same test. You can use the `SLD_EXTERNAL_LINKER` environment variable to run test scripts with a different linker:
...
## Building sld with sld

You can add or modify a `.cargo/config.toml` file to change the linker used to build `sld` to be `sld`!
...
rustflags = ["-C", "relocation-model=static", "-C", "link-arg=--ld-path=sld"]

[target.x86_64-unknown-linux-gnu]
linker = "/usr/bin/clang"
rustflags = ["-C", "link-arg=--ld-path=sld"]
```
